...
# Adding the description of the classification
ASTK_desc <- read_delim(file.path(dataFolder, "Arealstatistik_Dokumentation.csv"))
deer_cleaned <- merge(x = deer_cleaned, y = ASTK_desc, by.x = "astk", by.y = "CODE", all.x = TRUE)
deer_cleaned <- deer_cleaned %>% relocate(astk, .before = LABEL_DE)
...
raster_vis <- merge(x = raster_vis, y = ASTK_desc, by.x = "layer", by.y = "CODE", all.x = TRUE)
...
ggplot()+
  geom_sf(data = bbox_sf)+
  geom_raster(data = raster_vis, aes(x = x, y = y, alpha = LABEL_DE))+
  coord_sf(datum = 2056)+
  theme_minimal()+
  ggtitle("Tracking points for roe deer")
