...
bbox_sf <- bbox_df %>% st_as_sf(coords = c('X2', 'X1'), crs = 2056)
...
deer_sf <- deer %>% st_as_sf(coords = c("y", "x"), crs = 2056, remove = FALSE)
ggplot()+
...
ggplot()+
  geom_line(data = deer, aes(x=DatetimeUTC, y = steplength, col= TierID))+
  ggtitle("Monitoring timespan for 15 different roe deer")+
  ylab("Steplegth [m]")+
  xlab("Deer")
...
deer <- deer %>% mutate(astk = as.vector(extract(ASTK_raster, st_as_sf(deer, coords = c("x", "y")) , 'simple')))
```
...
deer_01 <- deer %>% filter(TierID == "RE01")
deer_02 <- deer %>% filter(TierID == "RE02")
deer_03 <- deer %>% filter(TierID == "RE03")
...
#Linear approach
fit <- lm(speed ~ sex + timelag + x + y + steplength + DatetimeUTC + astk, data = deer)
summ(fit)

effect_plot(fit, pred = astk, interval = TRUE, rug = TRUE)
