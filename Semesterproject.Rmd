...
# deer <- h2o.importFile(file.path(dataFolder, "all_deer_complete.csv")) #https://docs.h2o.ai/h2o/latest-stable/h2o-docs/data-science/deep-learning.html
...
#Rename deer as numeric 
```{r}
deer <- deer %>%
    mutate(TierID = recode(TierID, RE01 = 1))
```
...
deer$FutLon <- as.numeric(as.character(deer$FutLon))
deer$FutLat <- as.numeric(as.character(deer$FutLat))
...
ggplot()+
  geom_line(data = deer, aes(x=time, y = speed, col= TierID))+
...
ggplot()+
  ...
  geom_path(data = deer, aes(x = x, y = y), alpha = 0.3)+
  coord_sf(datum = 2056)+
  theme_minimal()+
...
trainingset <- dplyr::select(trainingset, -c(time, polar, displacement, FutLat, FutLon))

trainingset_h2o <- trainingset %>% as.h2o()

testset <- dplyr::select(testset, -c(time, polar, displacement, FutLat, FutLon))

testset_h2o <- testset %>% as.h2o()
```
...
# Second ANN method
model1 = neuralnet(x ~ y + sex + timelag + speed + steplength + displacementTime ,data=trainingset, hidden=79, act.fct = "logistic", 
                linear.output = FALSE, stepmax = 1000, rep = 100)

model2 = neuralnet(y ~ x + sex + timelag + speed + steplength + displacementTime ,data=trainingset, hidden=79, act.fct = "logistic", linear.output = FALSE, stepmax = 1000, rep = 100)

## Prediction using neural network
Predict_x=compute(model1, testset)
Predict_y=compute(model2, testset)

Predict_x$net.result
Predict_y$net.result


model_h2o = h2o.deeplearning(x = c(y, sex, timelag, speed, steplength, displacementTime), y = x, training_frame = trainingset_h2o, validation_frame = testset_h2o, activation = "Rectifier", hidden = rep(150,3), epochs = 1000)
