...
canton_zh <- canton_zh %>% st_set_crs(2056)

plot(canton_zh$geometry)

ASTK <- ASTK %>% st_as_sf(coords = c("N", "E"), crs = 2056, remove = FALSE) %>% st_transform(crs = 2056)
ASTK_ZH <- st_contains(canton_zh, ASTK)
ASTK_ZH <- ASTK_ZH %>% data.frame()
...
```{r}
deer <- deer %>% st_as_sf(coords = c("N", "E"), remove = FALSE) %>% st_set_crs(2056)
...
trainingset_clean <- select(trainingset, -c(DatetimeUTC, TierID, latitude, longitude)) %>% na.omit()
testset_clean <- select(testset, -c(DatetimeUTC, TierID, latitude, longitude)) %>% na.omit()
```
